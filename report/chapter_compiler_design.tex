...
The concrete syntax is described by an ANTLR grammar~\cite{parr2013antlr}. ANTLR generates a Python parser, which produces a parse tree for a module. The compiler then visits this parse tree and constructs a smaller abstract syntax tree (AST).
...
\Cref{lst:squaresum-ir} shows the pretty-printed IR for the square-sum program from \Cref{lst:squaresum-dsl}, compiled for \(GF(2^{116}-3)\) on the two-lane NEON backend with vectorization, schoolbook arithmetic, fully delayed limb realignment, and unrolling factor one. The source-level sum becomes a vectorized main loop over matrix values, followed by a scalar tail for the remaining element. Since realignment is fully delayed, the loop contains no intermediate carry instruction; the result is normalized by \texttt{full\_reduction} immediately before the store.
...
\Cref{lst:carry-delay-schedule} shows the difference schematically. In partially delayed mode, the compiler emits a carry after each unrolled batch of logical iterations. In fully delayed mode, the loop contains no intermediate carry instructions and all normalization is postponed to the final reduction.

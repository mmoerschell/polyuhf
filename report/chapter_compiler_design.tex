...
From these inputs it produces generated C sources and headers, which it then hands over to a C/C++ compiler (GCC or Clang) and build system (CMake, Ninja). If requested, it also emits Boost-based correctness tests and a standalone performance harness. \Cref{fig:compiler-overview} summarizes the pipeline.

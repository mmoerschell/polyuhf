...
Second, the performance model could be extended to account for carry propagation, full reductions, and Karatsuba multiplication more explicitly. The current model focuses on the dominant vectorized arithmetic in the main loop and assumes schoolbook multiplication for its multiplication cost. This is sufficient for the experiments in this thesis, but carry placement and the choice of multiplication algorithm are central optimization decisions in prime-field hashing, and modeling them directly would make the cost estimate more informative.

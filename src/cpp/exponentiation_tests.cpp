...
BOOST_DATA_TEST_CASE(RandomExpTests, boost::unit_test::data::xrange(10000), i) {
    std::mt19937 rng(42 + i); // deterministic per test case
    std::uniform_int_distribution<uint64_t> dist_limb_bits(0, LAMBDA_MASK);
    std::uniform_int_distribution<uint32_t> dist_exponent(0, 2);
    ...
    ref_b = powm(ref_a, exponent, c_1305);

...
BOOST_AUTO_TEST_CASE(SimpleMultiplication1) {
    bigint_t a{}, b{}, c{};
    a.limbs[0] = 42;
    b.limbs[0] = 24;
    c = multiplication(a, b);
    BOOST_CHECK_EQUAL(c.limbs[0], 42*24);
    for (size_t i = 1; i < LIMBS; ++i)
        BOOST_CHECK_EQUAL(c.limbs[i], 0);
}

BOOST_AUTO_TEST_CASE(MultCarry) {
    bigint_t a{}, b{}, c{};
    a.limbs[0] = LAMBDA_MASK;
    b.limbs[0] = 8;
    c = multiplication(a, b);
    BOOST_CHECK_EQUAL(c.limbs[0], LAMBDA_MASK & (LAMBDA_MASK << 3));
    BOOST_CHECK_EQUAL(c.limbs[1], 7);
    for (size_t i = 2; i < LIMBS; ++i)
        BOOST_CHECK_EQUAL(c.limbs[i], 0);
}

BOOST_DATA_TEST_CASE(RandomMultiplicationTests, boost::unit_test::data::xrange(10000),
                     i) {
    std::mt19937 rng(42 + i); // deterministic per test case
    std::uniform_int_distribution<uint32_t> dist(0, LAMBDA_MASK);

    // Create and multiply two random numbers
    alignas(64) bigint_t a{}, b{}, c{};
    for (size_t i = 0; i < LIMBS; ++i) {
        a.limbs[i] = dist(rng);
        b.limbs[i] = dist(rng);
    }
    a.limbs[LIMBS - 1] &= LAMBDA_PRIME_MASK;
    b.limbs[LIMBS - 1] &= LAMBDA_PRIME_MASK;
    c = multiplication(a, b);

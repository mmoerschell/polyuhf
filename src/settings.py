...
class BigIntConfiguration:
    ...
    def generate_header(self, output_path: str) -> None:
        ...
        lines: List[str] = [
            ...
            "#include <stdint.h>",
            "",
            f"#define LIMBS {self.limbs}UL",
            f"#define LAMBDA {self.lambd}UL",
            f"#define LAMBDA_PRIME {self.lambd_prime}UL",
            f"#define LAMBDA_MASK {lambd_mask}UL",
            f"#define LAMBDA_PRIME_MASK {lambd_prime_mask}UL",
            "",
            "typedef union {",
